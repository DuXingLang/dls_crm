...
<body>
   <center>
		...
		<hr>
		<br>
			<form action="/dls_crm/Activity_noteServlet" method="post">
				请输入要查询的信息：
				<input type="text" name="keyword">
				<input type="submit" name="查询">
			</form>
			<h3>
				<a href="${ctx }/Activity_noteServlet?method=createForm">新增活动信息</a>
			</h3>		
		...
		<tr>
			...
			<td>${note.getDate()}</td>
			<td>${note.getActivity()}</td>
			<td><a href="${ctx }/Activity_noteServlet?method=editForm&id=${note.getId()}">修改</a></td>
			<td><a href="${ctx }/Activity_noteServlet?method=delete&id=${note.getId()}">删除</a></td>
		</tr>
		</c:forEach>
		</table>
        </center>
